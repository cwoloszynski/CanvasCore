...
public final class APIClient: CanvasKit.APIClient {
	...
	public override func shouldComplete<T>(_ request: URLRequest, response: HTTPURLResponse?, data: Data?, error: NSError?, completion: ((Result<T>) -> Void)?) -> Bool {
		...
		return super.shouldComplete(request, response: response, data: data, error: error, completion: completion)
	}
}
